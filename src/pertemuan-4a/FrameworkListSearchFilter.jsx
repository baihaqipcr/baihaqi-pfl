...
export default function FrameworkList() {
  ...
  /* Filter logic yang sudah diperbaiki & disatukan */
  const filteredFrameworks = frameworkData.filter(item => {
    ...
    const matchSearch =
      ...
      item.tags?.some(t => t.toLowerCase().includes(_search));

    const matchFilter =
      selectedTag === "Semua" || item.tags?.includes(selectedTag);
        ...
        <div className="fl-inner">
          ...
          <div className="fl-header">
            ...
            <p className="fl-subtitle">
              Jelajahi koleksi framework modern dengan tampilan glassmorphism iOS 26
            </p>
          </div>
          ...
          <div className="fl-filters">
            {allTags.slice(0, 10).map(tag => (
              <FilterChip
                key={tag}
                label={tag}
                active={selectedTag === tag} /* PERBAIKAN: sele -> selectedTag */
                name="selectedTag"
                onChange={handleChange}
              />
            ))}
          ...
          <div className="fl-count">
            {/* PERBAIKAN: filtered -> filteredFrameworks */}
            Menampilkan {filteredFrameworks.length} dari {frameworkData.length} framework
          </div>
          ...
          {filteredFrameworks.length > 0 ? (
            ...
            <div className="fl-empty">
              ...
              {/* PERBAIKAN: search -> searchTerm */}
              <p>Tidak ada framework yang cocok dengan pencarian <strong>"{searchTerm}"</strong></p>
            </div>
          )}
